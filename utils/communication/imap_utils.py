...
class LinkExtractor:
    ...
    def __init__(
        ...
        self.link_patterns = [
            r"https://api\.dashboard\.3dos\.io\?p=[^\"'\s]+",
            r"https://mandrillapp\.com/track/click/\d+/[a-zA-Z0-9.-]+(?:\?p=[^\"'\s]+)?"
        ]
    ...
    def _collect_messages(self, mailbox: MailBox):
        ...
        for msg in mailbox.fetch(reverse=True, criteria=AND(from_="noreply@3dos.io"), limit=10, mark_seen=True):
        ...
        for msg in mailbox.fetch(reverse=True, limit=10, mark_seen=True):
            if msg.from_.startswith("noreply_at_3dos_io") or msg.from_ == "noreply@3dos.io":
                if self.redirect_email and self.redirect_email != msg.to[0]:
                    continue
